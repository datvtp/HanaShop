<%-- 
    Document   : home
    Created on : Feb 12, 2020, 2:37:10 PM
    Author     : vanth
--%>
            ...
            <c:forEach begin="1" end="${requestScope.NUMBEROFPAGE}" var="i">
                <div style="float: left; padding-left: 20px;s">
                    <c:if test="${(param.txtPageNumber) == i}">
                        <h1><a href="guestSearch?txtFoodName=${param.txtFoodName}&txtPageNumber=${i}&txtCategory=${param.txtCategory}&txtPriceMin=${param.txtPriceMin}&txtPriceMax=${param.txtPriceMax}">${i}</a></h1>
                        </c:if>
                        <c:if test="${(param.txtPageNumber) != i}">
                        <h3><a href="guestSearch?txtFoodName=${param.txtFoodName}&txtPageNumber=${i}&txtCategory=${param.txtCategory}&txtPriceMin=${param.txtPriceMin}&txtPriceMax=${param.txtPriceMax}">${i}</a></h3>
                        </c:if>
                </div>
            </c:forEach>
        </c:if>
