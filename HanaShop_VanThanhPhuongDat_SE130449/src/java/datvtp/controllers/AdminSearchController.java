...
public class AdminSearchController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            int totalRecord = foodDAO.getNumberOfFoodForAdmin(foodName, categoryId, minPrice, maxPrice);
            int numberOfPage = totalRecord / 20;//
            if (totalRecord > numberOfPage * 20) {//
                numberOfPage += 1;
            }
            int offsetRecord = (pageNumber - 1) * 20;//
            List<Tbl_FoodDTO> listFood = foodDAO.getListFoodSearchForAdmin(foodName, categoryId, minPrice, maxPrice, offsetRecord, 20);//
            request.setAttribute("LISTFOOD", listFood);
            request.setAttribute("NUMBEROFPAGE", numberOfPage);

        } catch (NumberFormatException e) {
            logger.error("ERROR NumberFormat at AdminSearchController: " + e.getMessage());
        } catch (SQLException e) {
            logger.error("ERROR SQL at AdminSearchController: " + e.getMessage());
        } catch (NamingException e) {
            logger.error("ERROR Naming at AdminSearchController: " + e.getMessage());
        } finally {
